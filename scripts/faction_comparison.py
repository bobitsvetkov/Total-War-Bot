...
class FactionComparisonBot(commands.Cog):
    ...
    async def compare_factions(self, faction1: str, faction2: str) -> str:
        ...
        stats_faction1 = analyze_faction_weights(tuple(make_hashable_unit(unit) for unit in self.factions[faction1]))
        stats_faction2 = analyze_faction_weights(tuple(make_hashable_unit(unit) for unit in self.factions[faction2]))
